...
enum LexToken LexScanGetToken(struct LexState *Lexer, struct Value **Value) {
	...
	do {
		*Value = &LexValue;
		...
		switch (ThisChar) {
			case '"':
				GotToken = LexGetStringConstant(Lexer, *Value, '"');
				break;
			case '\'': //'
...
enum LexToken LexGetRawToken(struct ParseState *Parser, struct Value **Value, int IncPos) {
	...
	do {
		...
		if (    Parser->FileName == StrEmpty
			...
			if (    InteractiveHead == NULL
			     || (unsigned char *)Parser->Pos == &InteractiveTail->Tokens[InteractiveTail->NumBytes-TOKEN_DATA_OFFSET]) {
	...
	} while (    (    Parser->FileName == StrEmpty
	               && Token == TokenEOF)
